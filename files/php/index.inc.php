...
<style>
		* {
		...
		body {
			...
			color: #6CAEE0;
            background-image: url('files/img/bkg/background.png');
		}
		.main {
			...
			box-shadow: 12px 12px 40px 0px rgba(116, 122, 128, 0.75);
		...
		<div class="main">
			...
			<div class="form-row">
				<label>
					...
					<input type="text"  autocomplete="off"  value="https://" name="<?php echo htmlspecialchars($GLOBALS['_config']['url_var_name']) ?>" value="<?php echo isset($_GET[$GLOBALS['_config']['url_var_name']]) ? htmlspecialchars(decode_url($_GET[$GLOBALS['_config']['url_var_name']])) : (isset($_GET['__iv']) ? htmlspecialchars($_GET['__iv']) : ''); ?>" placeholder="http://examp.le/" required="required"/>
				</label>
			</div>
			<div class="form-row">
				<button class="button-submit" type="submit">Go!</button>
				<label class="button-cancel" for="proxopttogl">Options</label>
			<a class="button-cancel" href="edit.php">User Agent</a>
			<a class="button-danger" href="logout.php">Log Out</a>
			<br>
<br><br><a href="http://github.com/JayAgra/plungeProxy"><center><img src="files/img/ghub.png" href="http://github.com/JayAgra/plungeProxy" width="10%"/></a></center><br><center><small><p>Derived from PHProxy</p></small></center>
			</div>
<?php
...
?>
				...
				<a href="edit.php">User Agent & Account...</a>
			</div>
		<?php endif;?>
</form>
